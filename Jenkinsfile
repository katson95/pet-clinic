        ...
        stage('Build and Test Image') {
            container('a-360') {
                stage('Package into Docker Image') {
                    ...
                    sh 'docker tag pet-clinic:latest nexus.ops.dev.invent-360.com:5000/katson95/pet-clinic:latest'
                }
            }
        }
            ...
            container('a-360'){  
                stage('Publish Image to Docker Registry') {
                  withCredentials([usernamePassword(credentialsId: 'i360-nexus-id', passwordVariable: 'dockerPassword', usernameVariable: 'dockerUsername')]) {
                   sh "docker login -u ${env.dockerUsername} -p ${env.dockerPassword} nexus.ops.dev.invent-360.com:5000"
                   sh "docker push nexus.ops.dev.invent-360.com:5000/${IMAGE_NAME}:${IMAGE_VERSION}"
                }
              }
            }
        }
